  ...
  return (
    ...
    >
          ...
          return (
            <Link
              ...
              className={cn(
                'sidebar-link',
                active && 'active',
                item.highlight && !active && 'text-purple-400/70',
              )}
            >
        ...
        {profile?.role === 'admin' && (
          <>
            <div className="pt-5 pb-2">
              <p className="px-3 text-[10px] font-semibold text-white/20 uppercase tracking-widest">Admin</p>
            </div>
            {adminItems.map((item) => {
      ...
      <div className="px-3 pb-4 pt-3 border-t border-white/[0.05]">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors hover:bg-white/[0.03] group">
          <div
            className="w-8 h-8 rounded-xl flex-shrink-0 flex items-center justify-center text-sm font-bold"
            style={{
              background: 'linear-gradient(135deg, rgba(123,47,247,0.4), rgba(6,182,212,0.3))',
              color: '#c4b5fd',
            }}
          >
